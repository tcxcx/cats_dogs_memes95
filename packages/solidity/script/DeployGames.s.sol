...
contract DeployGames is Script {
  ...
  function createCards() public {
    ...
    // note that for now the uri and data will not align. 
    ...
    // CardData[2] = Cards.Card({name: 'Pomeranian' , cardType:'Dog'	, atk: 13	, hp: 1	, spd:6	, infRange:333	, supRange:426}); 
    // CardData[3] = Cards.Card({name: 'Miniature Schnauzer' , cardType:'Dog'	, atk: 6	, hp: 7	, spd:2	, infRange:426	, supRange:472}); 
    // CardData[4] = Cards.Card({name: 'Baby Doge Coin' , cardType:'Meme'	, atk: 0	, hp: 10	, spd:10	, infRange:667	, supRange:759}); 
    // CardData[5] = Cards.Card({name: 'Maga' , cardType:'Meme'	, atk: 1	, hp: 1	, spd:1	, infRange:759	, supRange:806}); 
    // CardData[6] = Cards.Card({name: 'Bonk' , cardType:'Meme'	, atk: 5	, hp: 9	, spd:3	, infRange:974	, supRange:980}); 
    // CardData[7] = Cards.Card({name: 'Dog Wif Hat' , cardType:'Meme'	, atk: 4	, hp: 6	, spd:6	, infRange:980	, supRange:985}); 
    // CardData[8] = Cards.Card({name: 'Pepe' , cardType:'Meme'	, atk: 8	, hp: 9	, spd:2	, infRange:985	, supRange:990}); 
    // CardData[9] = Cards.Card({name: 'Shiba Inu' , cardType:'Meme'	, atk: 8	, hp: 9	, spd:2	, infRange:990	, supRange:995}); 
   
    // //message: We still have to decide on rarity of cards
    // uint256[] memory mintAmounts = new uint256[](CardData.length); 
    // for (uint256 i; i < CardData.length; i++) { 
    //   mintAmounts[i] = 50 + (i * 25); 
    // }  
