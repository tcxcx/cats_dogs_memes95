...
```bash
# For Linux
docker build -t cats-dogs-memes-etc:latest .

# For Mac with Apple Silicon chips
docker buildx build --platform linux/amd64,linux/arm64 -t cats-dogs-memes-etc:latest .
```
...
docker run --env-file .env -v ./db.sqlite:/app/db.sqlite -p <HOST_PORT>:<CONTAINER_PORT> --name=cats-dogs-memes-etc -it cats-dogs-memes-etc:latest

# If using other URI based datastores
docker run --env-file .env -p <HOST_PORT>:<CONTAINER_PORT> --name=cats-dogs-memes-etc -it cats-dogs-memes-etc:latest
```
