...
import {VRFV2PlusWrapperConsumerBase} from
    "../lib/chainlink/contracts/src/v0.8/vrf/dev/VRFV2PlusWrapperConsumerBase.sol";
...
contract Cards is ERC1155, VRFV2PlusWrapperConsumerBase, ConfirmedOwner {
     ...
     */
    function openCardPack(uint256 cardPackNumber)
        public
        payable
     ...
     *
     */
    function transferCards(address recipient, uint256[] memory cardIds, uint256[] memory cardvalues)
        public
        payable
        onlyOwner
    {
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        for (uint256 i; i < NUMBER_CARDS_IN_PACK; i++) {
            uint256 pseudoRandomNumber = uint256(keccak256(abi.encode(_randomWords[i] + s_cardPackNumber)))
    ...
    function _pseudoRandomiser(uint256 salt) internal view returns (uint256 pseudoRandomNumber) {
        pseudoRandomNumber =
            uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, blockhash(block.number), salt)));
