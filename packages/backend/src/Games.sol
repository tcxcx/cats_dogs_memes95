...
contract Games {
    ...
    struct Game {
        // The Game struct stores the two Players that challenges each other and their cardecks, store the state of the game, its winner and a unique nonce.
        address playerOne;
        address playerTwo;
        uint256[] cardDeckOne;
        uint256[] cardDeckTwo;
    ...
    struct Player {
        // The Player struct stores the cardDeck to the player, the tournament their playing in, accumulated score and state.
        uint256 tournament;
        uint256 score;
        Status status;
    }
    ...
    function startTournament() public onlyOwner {
        if (s_statusTournament != Status.Idle && s_statusTournament != Status.Completed) {
    ...
    //
    function joinGame(address opponent, uint256 nonce, uint256[] memory cardDeck)
        public
    ...
    function completeGame(uint256 nonce, address playerOne, address winner)
        public
        onlyDuringActiveTournament
        onlyAvatarBasedAccount
    {
        bytes32 gameHash = keccak256(abi.encode(playerOne, nonce));
        if (s_games[gameHash].status != Status.Active && s_games[gameHash].status != Status.Paused) {
            revert Games__GameNotActiveOrPaused();
        }
        if (msg.sender != s_games[gameHash].playerOne && msg.sender != s_games[gameHash].playerTwo) {
            revert Games__SenderNotPlayerInGame();
        }
        if (s_players[msg.sender].status != Status.Active) {
    ...
    /* internal */
    function _checkDeckAgainstCollection(address player, uint256[] memory cardDeck)
        internal
        view
    ...
    function _registerPlayerInTournament(address avatarBasedAccount) internal {
        ...
        Player memory player = Player({tournament: s_tournamentCounter, score: 0, status: Status.Idle});
    ...
    function getRankings()
        ...
        view
        onlyDuringActiveTournament
        returns (address[] memory avatars, uint256[] memory scores, uint256[] memory rankings)
    {
