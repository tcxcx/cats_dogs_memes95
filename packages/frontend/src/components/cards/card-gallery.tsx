...
export function CardGalleryComponent({
  card,
  onAddToDeck,
  inDeck,
}: {
  ...
  return (
    <div className="relative w-64 mx-auto">
      ...
      <Card
        className={`relative w-full h-96 ${cardColor} border-4 border-black rounded-2xl overflow-hidden`}
      >
        {card.count > 1 && !inDeck && (
          ...
          <div className={`${cardColor} border-2 border-gray-700 px-2 py-1`}>
            <div className="relative text-center focus:ring-0 transform bg-white px-1 py-1 rounded-full border-2 border-gray-700 z-10 mb-2">
              <span
                ...
                aria-label={`Card subtype: ${card.subtype}`}
              >
                {card.subtype}
              </span>
            </div>
      ...
      {onAddToDeck && (
        <Button
          ...
          className={`w-full mt-2 ${
            inDeck
              ? "bg-gray-400"
              : `${cardColor} text-black hover:${cardColor} hover:brightness-90`
          }`}
          disabled={inDeck}
        >
