...
export function CardSmall({
  ...
  return (
    <div className="relative mx-auto text-xs">
      <div
        className={`absolute text-xs top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-cyan-200 px-4 py-1 rounded-full border-2 border-black z-1`}
      >
        <span className="hidden">1st Edition</span>
      </div>
      <Card2
        className={`relative w-full h-auto ${cardColor} border-4 border-black/5 rounded-2xl overflow-hidden`}
      >
        {card.count > 1 && !inDeck && (
